...
url = 'https://consultation.education.govt.nz'
colour = '#262739'

async def scrape_moe_consultations(get_html_func=_get_soup_urllib) -> list[dict]:
	...
	for descendant in current.parent.parent.parent.ul.descendants:
		...
		for html_entry in html_entries:
			...
			entries.append(dict(org=polis.SCRAPER_ENUM.moe, title=title, url=url, blurb=blurb, date=date_obj))
