...
url = 'https://consultations.justice.govt.nz/'
colour = '#263E78'

async def scrape_moj_consultations(get_html_func=_get_soup_urllib) -> list[dict]:
	...
	for descendant in current.parent.parent.parent.ul.descendants:
		...
		for html_entry in html_entries:
			...
			entries.append(dict(org=polis.SCRAPER_ENUM.moj, title=title, url=url, blurb=blurb, date=date_obj))
